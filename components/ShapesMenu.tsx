        ...
        <DropdownMenuContent className="mt-5 flex flex-col gap-y-1 border-none bg-black py-4 text-white">
          {item.value.map((elem) => (
            <Button
              ...
              className={`flex h-fit justify-between gap-10 rounded-none px-5 py-3 focus:border-none ${activeElement.value === elem?.value ? "bg-green" : "hover:bg-gray-200"
                }`}
            >
              <div className="group flex items-center gap-2">
                ...
                />
                <p
                  className={`text-sm  ${activeElement.value === elem?.value ? "text-black" : "text-white"
                    }`}
                >
                  {elem?.name}
                </p>
